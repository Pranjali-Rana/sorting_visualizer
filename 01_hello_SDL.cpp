...
{
    SDL_SetRenderDrawColor(renderer, 0, 0, 0, 0); //seting bg black
    SDL_RenderClear(renderer);  //clear screen

    int j=0;
    for(int i=0; i<=SCREEN_WIDTH-rectSize; i+=rectSize)
    {
        SDL_PumpEvents(); //process inp events

        SDL_Rect rect={i, 0, rectSize, arr[j]};
        if(complete)
        {
            SDL_SetRenderDrawColor(renderer, 100, 180, 100, 0);  //when sorting is done set color to green
